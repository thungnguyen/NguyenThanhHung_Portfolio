...
import { Github, Linkedin, FileText, ArrowRight, Mail, Eye } from 'lucide-react';
...
const Hero = ({ lang, t }) => {
  ...
  return (
    <section
      id="hero"
      className="min-h-screen relative flex items-center justify-center pt-24 md:pt-28 pb-8 md:pb-16 overflow-hidden bg-slate-50 dark:bg-darkBg transition-colors duration-300"
    >
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 z-10 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center w-full">
        ...
        >
          <h1 className="font-display font-extrabold text-3xl sm:text-4xl md:text-5xl lg:text-6xl tracking-tight leading-tight text-slate-800 dark:text-white mb-6 text-left flex flex-wrap items-center justify-start">
            <span className="font-semibold text-2xl sm:text-3xl md:text-4xl text-slate-500 dark:text-gray-400 mr-3">
              {t.hero.greeting}
            </span>
          ...
          {/* Action Buttons */}
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-start gap-4 mb-8">
            <a
              href="#projects"
              onClick={(e) => handleScrollTo(e, 'projects')}
              className="group flex items-center gap-2 px-6 py-3.5 w-full sm:w-auto justify-center rounded-xl bg-gradient-to-r from-accentTeal to-accentIndigo text-white font-semibold text-sm shadow-md hover:shadow-lg shadow-accentTeal/20 hover:shadow-accentTeal/30 hover:scale-[1.02] active:scale-[0.98] transition-all cursor-pointer"
            >
              {t.hero.ctaProjects} <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
            </a>

            <a
              href="#cv-viewer"
              onClick={(e) => handleScrollTo(e, 'cv-viewer')}
              className="group flex items-center gap-2 px-6 py-3.5 w-full sm:w-auto justify-center rounded-xl glass border border-slate-200 dark:border-white/10 hover:border-slate-300 dark:hover:border-white/20 text-slate-700 dark:text-white font-semibold text-sm hover:bg-slate-100 dark:hover:bg-white/5 hover:scale-[1.02] active:scale-[0.98] transition-all cursor-pointer shadow-sm"
            >
              <Eye className="w-4 h-4 text-accentTeal group-hover:scale-110 transition-transform" /> {t.hero.ctaViewCv}
            </a>

            <a
              href="/NguyenThanhHung_SoftwareEngineer_CV.pdf"
              download
              className="group flex items-center gap-2 px-6 py-3.5 w-full sm:w-auto justify-center rounded-xl border border-accentTeal/20 hover:border-accentTeal/50 text-accentTeal hover:text-white bg-accentTeal/5 hover:bg-accentTeal/90 hover:scale-[1.02] active:scale-[0.98] transition-all duration-300 font-semibold text-sm cursor-pointer"
            >
              <FileText className="w-4 h-4 group-hover:-translate-y-0.5 transition-transform" /> {t.hero.ctaDownloadCv}
            </a>
          </div>
        ...
        >
          ...
          <div className="relative group/avatar">
            ...
            {/* Tech Float items */}
            <div className="absolute -top-2 -right-4 px-4 py-2.5 glass rounded-2xl border border-slate-250/30 dark:border-white/10 shadow-xl shadow-accentTeal/5 dark:shadow-black/40 animate-float flex items-center gap-2">
            ...
            <div className="absolute -bottom-2 -left-4 px-4 py-2.5 glass rounded-2xl border border-slate-250/30 dark:border-white/10 shadow-xl shadow-accentIndigo/5 dark:shadow-black/40 animate-float flex items-center gap-2" style={{ animationDelay: '2s' }}>
              <span className="w-2.5 h-2.5 rounded-full bg-accentIndigo shadow-lg shadow-accentIndigo/50 animate-pulse" />
              <div className="flex flex-col items-start leading-none">
                <span className="text-[9px] font-bold text-slate-500 dark:text-gray-400 uppercase tracking-widest mb-0.5">Restful</span>
                <span className="text-sm sm:text-base font-extrabold text-slate-800 dark:text-white">API</span>
              </div>
            </div>

            <div className="absolute bottom-10 -right-6 px-4 py-2.5 glass rounded-2xl border border-slate-250/30 dark:border-white/10 shadow-xl shadow-cyan-500/5 dark:shadow-black/40 animate-float flex items-center gap-2" style={{ animationDelay: '4s' }}>
              <span className="w-2.5 h-2.5 rounded-full bg-cyan-400 shadow-lg shadow-cyan-400/50 animate-pulse" />
              <div className="flex flex-col items-start leading-none">
                <span className="text-[9px] font-bold text-slate-500 dark:text-gray-400 uppercase tracking-widest mb-0.5">Integrate</span>
                <span className="text-sm sm:text-base font-extrabold text-slate-800 dark:text-white">AI</span>
              </div>
            </div>
          </div>
        </motion.div>
